...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    if (contentType.includes('image/jpeg') || contentType.includes('image/jpg')) {
      // Raw JPEG bytes posted directly from ESP32-CAM
      const buf = await readRawBody(req);
      imageBase64 = buf.toString('base64');
      device_id   = req.headers['x-device-id'] || '';
    } else {
      // JSON body { imageBase64, device_id }
      const buf  = await readRawBody(req);
...
Set triggerCleaning to true ONLY for heavily_dusty or bird_dropping conditions.`;
    ...
    const result = { ...analysis, device_id, timestamp: new Date().toISOString() };

    // Persist to Supabase if configured
    if (supabase && device_id) {
